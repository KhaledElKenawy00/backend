...
class ThreadService {
  ...
  Future<MessageModel> sendMessage(String channelId, String threadId, String content) async {
    final headers = await _apiClient.chatServiceHeaders();
    final uri = Uri.parse('${ApiConstants.chatServiceBase}/api/chat/channels/$channelId/messages');
    final response = await http.post(uri, headers: headers,
        body: jsonEncode({'content': content, 'threadId': threadId}));
